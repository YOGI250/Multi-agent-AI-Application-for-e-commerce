...
def get_policy(issue_type: str) -> Optional[dict]:
    ...
    db = SessionLocal()
    try:
        policy = db.query(Policy).filter(Policy.issue_type == issue_type).first()
...
def get_user_complaint_history(user_id: str, order_id: Optional[str] = None) -> dict:
    ...
    db = SessionLocal()
    try:
        query = db.query(SupportTicket).filter(SupportTicket.user_id == user_id, SupportTicket.status == "open")
        if order_id:
        ...
        if ticket:
            ...
            logger.info(f"Open ticket found: {ticket.ticket_id} for user {user_id}, " f"days_open={days_open}")
            return {"existing_ticket_id": ticket.ticket_id, "is_duplicate": True, "days_open": days_open}
...
def create_ticket(user_id: str, issue_type: str, priority: str, order_id: Optional[str] = None) -> dict:
    """
    ...
    try:
        ...
        existing_query = db.query(SupportTicket).filter(
            SupportTicket.user_id == user_id, SupportTicket.issue_type == issue_type, SupportTicket.status == "open"
        )
        if order_id:
            existing_query = existing_query.filter(SupportTicket.order_id == order_id)
        ...
        logger.info(f"Ticket created: {ticket.ticket_id} " f"for user {user_id} with priority {priority}")
