...
def generate_tracking_events(status, order_date, expected_delivery):
    ...
    if status in ["shipped", "delivered", "delayed"]:
        ...
        events.append({
            "timestamp": str(base_time + timedelta(hours=14)),
            "location": "Origin Hub",
            "status": "In transit"
        })
...
def import_orders(db):
    ...
    shipped_sample = orders_df[
        orders_df['clean_status'] == 'shipped'
    ].sample(n=min(10, len(orders_df[orders_df['clean_status'] == 'shipped'])), random_state=99)
    ...
    for idx, (_, row) in enumerate(sampled.iterrows()):
        try:
            ...
            order_date = pd.to_datetime(
                row['order_purchase_timestamp'], errors='coerce'
            )
            ...
            expected_del = pd.to_datetime(
                row['order_estimated_delivery_date'], errors='coerce'
            )
            if pd.isna(expected_del):
                expected_delivery = (
                    order_date + timedelta(days=random.randint(5, 15))
                ).date()
            else:
            ...
            items = [
                {
                    ...
                    "qty": 1,
                    "price": round(float(row['order_value']), 2)
                }
            ]
            ...
            if status not in ['processing', 'cancelled']:
                events = generate_tracking_events(
...
def seed_demo_tickets(db):
    ...
    existing = db.query(SupportTicket).filter(
        SupportTicket.user_id == DEMO_USER_ID
    ).count()
