...
class TestChatEndpoint:
    ...
    def test_chat_returns_200_for_guest(self, client, mock_db_session, mock_agent_result):
        mock_user = MagicMock()
    ...
    def test_chat_returns_response_field(self, client, mock_db_session, mock_agent_result):
        mock_session = MagicMock()
        mock_session.is_active = True
        mock_session.last_active_at = MagicMock()
...
class TestSessionManagement:
    ...
    def test_resolve_session_loads_existing_session(self, mock_db_session):
        ...
        mock_session.is_active = True
        mock_session.last_active_at = datetime.now()
        ...
        result = resolve_session("session_abc", "user_123", mock_db_session)
    ...
    def test_resolve_session_expires_stale_session(self, mock_db_session):
        ...
        mock_session.last_active_at = datetime.now() - timedelta(hours=48)
...
class TestAdditionalEndpoints:
    ...
    def test_session_close_when_session_not_found(self, client, mock_db_session):
        ...
        response = client.post("/session/close", headers={"X-Session-ID": "non_existent_session"})
        assert response.status_code == 200
        assert response.json()["closed"] == False
    ...
    def test_session_close_when_session_found(self, client, mock_db_session):
        ...
        response = client.post("/session/close", headers={"X-Session-ID": "active_session_123"})
        assert response.status_code == 200
    ...
    def test_history_session_not_found(self, client, mock_db_session):
        ...
        response = client.get("/history", headers={"X-Session-ID": "missing_session"})
        assert response.status_code == 200
    ...
    def test_history_returns_messages(self, client, mock_db_session):
        ...
        mock_db_session.query.return_value.filter.return_value.order_by.return_value.all.return_value = [mock_msg]

        response = client.get("/history", headers={"X-Session-ID": "session_with_messages"})
        assert response.status_code == 200
