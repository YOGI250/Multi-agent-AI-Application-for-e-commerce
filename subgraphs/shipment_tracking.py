...
def get_carrier_info(state: ShipmentTrackingState) -> ShipmentTrackingState:
    ...
    if span:
        end_span(span, {"carrier_name": state["carrier_name"], "tracking_number": state["tracking_number"]})

    return state


def fetch_tracking_data_node(state: ShipmentTrackingState) -> ShipmentTrackingState:
    ...
    if span:
        end_span(
            span,
            {"status": (tracking or {}).get("current_status"), "location": (tracking or {}).get("current_location")},
        )
...
def parse_eta(state: ShipmentTrackingState) -> ShipmentTrackingState:
    ...
    span = (
        create_span(
            trace_id=trace_id, name="parse_eta", parent_observation_id=parent_id, input_data={"tracking_data": tracking}
        )
        if trace_id
        else None
    )
    ...
    if span:
        end_span(span, {"eta": state["eta"], "current_location": state["current_location"]})
