...
def fetch_tracking_data(tracking_number: str) -> dict:
    ...
    """
    logger.info(f"Tool called: fetch_tracking_data with " f"tracking_number={tracking_number}")
    ...
    logger.info(f"Tracking fetched: {tracking_number} " f"status={result['current_status']}")
    return result
...
def fetch_all_orders_for_user(user_id: str) -> list:
    ...
    """
    logger.info(f"Tool called: fetch_all_orders_for_user " f"with user_id={user_id}")
    try:
        ...
        db.close()
        logger.info(f"fetch_all_orders_for_user found " f"{len(result)} orders for {user_id}")
        return result
