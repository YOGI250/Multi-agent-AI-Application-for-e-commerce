...
def check_user_history_tool(user_id: str, order_id: Optional[str] = None) -> dict:
    ...
    """
    logger.info(f"Tool called: check_user_history_tool for " f"user_id={user_id} order_id={order_id}")

    result = get_user_complaint_history(user_id, order_id=order_id)

    logger.info(f"History for {user_id}: is_duplicate={result['is_duplicate']}, " f"days_open={result['days_open']}")
...
def create_ticket_tool(user_id: str, issue_type: str, priority: str, order_id: Optional[str] = None) -> dict:
    ...
    """
    logger.info(f"Tool called: create_ticket_tool for user={user_id} " f"issue={issue_type} priority={priority}")
